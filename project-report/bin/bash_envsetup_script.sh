...
# The script installs MongoDB on ubuntu 18.04 and download and imports the dataset(kaggle) to MongoDB.
# The script must run as root user or user that has sudo privileges to install 
...
echo "if kaggle api package not installed use pip install kaggle" 
...
echo "Install MongoDB package default installs the latest version"
...
echo "records the mongodb status using mongo shell"
